...
class Dict(dict):
    '''
    字典对象，实现一个简单的可以通过属性访问的字典，如x.key=value
...
class _LasyConnection(object):
    '''
    惰性连接对象，仅当需要cursor对象时才连接数据库，获取连接
    '''
    def __init__(self):
...
class _DbCtx(threading.local):
    ...
    取得的连接时惰性连接对象，因此只有调用cursor对象时，才会真正获取数据库连接
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    '''
    db模型的核心函数，用于连接数据库，生成全局对象engine，此对象持有数据库连接
    '''
...
class _ConnectionCtx(object):
    '''
    因为_DbCtx实现了连接的获取和释放，但并没有实现连接的自动获取和释放，
    _ConnectCtx在_DbCtx的基础上实现了该功能，因此可以对 _ConnectCtx使用with语法
...
def connection():
    ...
    通过_ConnectionCtx对 _db_ctx封装，使得惰性连接可以实现
...
class _TransactionCtx(object):
    '''
    事务嵌套比Connection嵌套复杂，因为事务嵌套需要计数，
    每遇到一层嵌套就+1，离开一层就-1，最后到0时提交事务
...
if __name__=='__main__':
    ...
    d = select('select * from test1')
    print d
    d = select_one('select * from test1')
    print d
    d = select_int('select count(*) from test1')
    print d
